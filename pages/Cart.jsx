...
export default function Cart() {
  ...
  const [cart, setCart] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("inquiryCart") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const getItemSpecsLines = (item) => {
    ...
    return { line1: line1 || item.specs || "—", line2 };
  };
  ...
  const buildWhatsAppMessage = () => {
    const productLines = cart
      .map((item, i) => {
        const image = getItemMainImage(item);
        return [
    ...
    const regionLabel = selectedRegion ? t[selectedRegion.labelKey] : region;
  ...
  // ── Item count label ──
  const itemCountLabel = t.itemCount.replace("{{count}}", cart.length);
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center px-6">
        <div className="flex flex-col items-center text-center max-w-md">
          ...
          <h2 className={`font-display text-2xl font-bold mb-2 ${c.text}`}>
            {t.emptyTitle}
          </h2>
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-6 sm:py-12">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 xl:gap-16">
        {/* ═══════ LEFT: Cart Items ═══════ */}
        <div className="xl:col-span-2 flex flex-col gap-6 sm:gap-10">
          {/* Header */}
          <div>
            <h1 className={`font-display text-2xl sm:text-3xl lg:text-4xl font-bold mb-2 ${c.text}`}>
              {t.title}
            </h1>
          ...
          <div className="flex flex-col gap-3 md:hidden">
            {cart.map((item, index) => (
              <div
                ...
                className={`rounded-2xl border p-4 ${c.card} ${c.cardShadow}`}
              >
                  ...
                  <div className={`h-16 w-16 shrink-0 overflow-hidden rounded-xl ${c.imgPlaceholder}`}>
                    {getItemImage(item) ? (
                      ...
                      <div className="flex h-full w-full items-center justify-center">
                        <span className={`text-xs ${c.textVeryFaint}`}>—</span>
                      </div>
                    )}
                  </div>
                  {/* Info */}
                  <div className="flex-grow min-w-0">
                    <p className={`font-display font-bold text-sm truncate ${c.text}`}>{item.name}</p>
                    <span className={`inline-block rounded-md px-2 py-0.5 font-mono text-[10px] font-bold mt-1 ${c.codeBadge}`}>
                      {getItemCode(item)}
                    </span>
                    {item.material && (
                      <p className={`text-[11px] mt-1 ${c.textFaint}`}>{item.material}</p>
                    )}
                  </div>
                  {/* Remove */}
                  <button
                    onClick={() => removeItem(index)}
                    className={`shrink-0 h-8 w-8 flex items-center justify-center rounded-full transition-all ${c.removeBtn}`}
                ...
                <div className="flex items-center justify-end mt-3 pt-3 border-t" style={{ borderColor: isDark ? '#1f2937' : '#e5e7eb' }}>
                  <div className={`inline-flex items-center rounded-full border overflow-hidden ${c.qtyBg}`}>
                    ...
                    <input
                      ...
                      min={1}
                      className={`h-8 w-10 bg-transparent text-center text-sm font-bold border-none outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${c.qtyInput}`}
                    />
                    <button onClick={() => updateQty(index, 1)} className={`flex h-8 w-8 items-center justify-center text-sm transition-colors ${c.qtyBtn}`}>+</button>
                  </div>
                </div>
          ...
          <div className={`hidden md:block rounded-3xl border overflow-hidden ${c.card} ${c.cardShadow}`}>
            <table className="w-full text-sm">
              <thead>
                <tr className={`border-b ${c.rowBorder}`}>
                  <th className={`py-4 px-6 text-left text-[10px] font-bold uppercase tracking-widest ${c.textFaint}`}>{t.tableProduct}</th>
                  <th className={`py-4 px-6 text-left text-[10px] font-bold uppercase tracking-widest ${c.textFaint}`}>{t.tableCode}</th>
                  <th className={`py-4 px-6 text-left text-[10px] font-bold uppercase tracking-widest ${c.textFaint}`}>{t.tableSpecs}</th>
                  ...
                  <tr key={item.product_id || index} className={`border-b last:border-0 transition-colors ${c.rowBorder} ${c.rowHover}`}>
                    <td className="py-5 px-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className={`font-display font-bold text-sm ${c.text}`}>{item.name}</p>
                          {item.material && <p className={`text-[11px] mt-0.5 ${c.textFaint}`}>{item.material}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="py-5 px-6">
                    ...
                    <td className="py-5 px-6">
                      <div className="flex items-center justify-center">
                        <div className={`inline-flex items-center rounded-full border overflow-hidden ${c.qtyBg}`}>
                          <button onClick={() => updateQty(index, -1)} className={`flex h-9 w-9 items-center justify-center transition-colors ${c.qtyBtn}`} aria-label="Decrease">−</button>
                          ...
                          <button onClick={() => updateQty(index, 1)} className={`flex h-9 w-9 items-center justify-center transition-colors ${c.qtyBtn}`} aria-label="Increase">+</button>
                        </div>
                      </div>
                    </td>
                    <td className="py-5 px-6 text-center">
                      <button onClick={() => removeItem(index)} className={`inline-flex h-9 w-9 items-center justify-center rounded-full transition-all ${c.removeBtn}`} aria-label="Remove item">
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className={`rounded-2xl sm:rounded-3xl p-5 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6 border ${c.browseCard}`}>
            <div>
              <h3 className={`font-display font-bold text-lg mb-1 ${c.text}`}>
                {t.continueBrowsingTitle}
              </h3>
              <p className={`text-sm ${c.textMuted}`}>
                {t.continueBrowsingDesc}
              </p>
            </div>
            <Link
              to="/catalogue"
              className={`inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-bold border transition-all shrink-0 ${c.browseBtn}`}
            >
              {t.viewCatalog}
          ...
          <div className="sticky top-24">
            <div className={`rounded-2xl sm:rounded-3xl border p-5 sm:p-8 flex flex-col gap-6 sm:gap-8 ${c.card} ${c.cardShadow}`}>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`inline-block rounded-full px-3 py-1 text-[11px] font-bold ${c.codeBadge}`}>
                  {itemCountLabel}
                </span>
              </div>
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-1.5">
                  <label
                    htmlFor="buyerName"
                    className={`text-[11px] font-bold uppercase tracking-widest ${c.label}`}
                  >
                    {t.buyerName}
                  </label>
                  <input
                    ...
                    className={`w-full rounded-2xl border px-4 py-3 text-sm font-medium outline-none focus:ring-2 focus:ring-[#0071e3]/30 transition-colors ${c.input}`}
                  />
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <textarea
                    ...
                    rows={4}
                    className={`w-full rounded-2xl border px-4 py-3 text-sm font-medium outline-none focus:ring-2 focus:ring-[#0071e3]/30 resize-none transition-colors ${c.input}`}
                  />
                </div>
              </div>
              ...
              {/* Form Error */}
              {formError && (
                <p className={`text-xs font-medium rounded-xl px-4 py-3 ${isDark ? "text-red-400 bg-red-900/30" : "text-red-500 bg-red-50"}`}>
                  {formError}
                </p>
              )}
              ...
              <button
                ...
                className="group relative w-full overflow-hidden rounded-full bg-[#25D366] p-5 text-sm font-bold text-white shadow-lg transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl active:scale-95 flex items-center justify-center gap-3"
              >
              ...
              <div className={`rounded-2xl p-4 border ${c.disclaimer}`}>
                <p className="text-[11px] text-center leading-relaxed">
                  {t.disclaimer}{" "}
                  <a href="#" className={`underline ${c.link}`}>
                    {t.termsOfService}
                  </a>
                </p>
              </div>
